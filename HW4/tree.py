...
class Node:
    ...
    def __init__(self, feature_index, threshold, height=0, proba=0):
        ...
        for feature in range(X_subset.shape[1]):
            for j in range(X_subset.shape[0]):
                y_left, y_right = self.make_split_only_y(
                    feature, X_subset[j, feature], X_subset, y_subset
                )
                if (
                    y_left.shape[0] < self.min_samples_split
                    or y_right.shape[0] < self.min_samples_split
                ):
                    continue
                G = (
                    criterion(y_subset)
                    - y_left.shape[0] / y_subset.shape[0] * criterion(y_left)
                    - y_right.shape[0] / y_subset.shape[0] * criterion(y_right)
                )
                if G > best_G:
                    best_G = G
                    feature_index = feature
                    threshold = X_subset[j, feature]

        return feature_index, threshold

    def make_tree(self, X_subset, y_subset, height=1):
        """
        ...
        if feature_index == -1 or height == self.max_depth:
            new_node = Node(None, None, height)
        ...
        new_node = Node(feature_index, threshold, height)

        new_node.left_child = self.make_tree(X_left, y_left, height + 1)
        new_node.right_child = self.make_tree(X_right, y_right, height + 1)
        ...
        for i in range(X.shape[0]):
            ...
            while initial.left_child is not None and initial.right_child is not None:
                if X[i, initial.feature_index] < initial.value:
                    ...
                    initial = initial.right_child
            y_predicted_probs[i] = initial.proba
