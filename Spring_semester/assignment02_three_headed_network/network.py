...
class ThreeInputsNet(nn.Module):
    def __init__(self, n_tokens, n_cat_features, concat_number_of_features, hid_size=64):
        ...
        self.title_nn = nn.Sequential(
            nn.Conv1d(in_channels=hid_size, out_channels=3*hid_size, kernel_size=3),
            nn.ReLU(),
            nn.AdaptiveAvgPool1d(1)
        )
        
        self.full_emb = nn.Embedding(num_embeddings=n_tokens, embedding_dim=hid_size)
        self.desc_nn = nn.Sequential(
            nn.Conv1d(in_channels=hid_size, out_channels=3*hid_size, kernel_size=3),
            nn.ReLU(),
            nn.AdaptiveAvgPool1d(1)
        )
        ...
        self.category_out = nn.Sequential(
            nn.Linear(in_features=n_cat_features, out_features=hid_size*3),
            nn.ReLU(),
        )
